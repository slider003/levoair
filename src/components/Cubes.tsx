...
const Cubes: React.FC<CubesProps> = ({
  ...
  // fixed columns/rows equal to gridSize (original behavior)
  const cols = gridSize;
  const rows = gridSize;
  ...
  // fixed grid: no resize recompute
  ...
  const wrapperStyle = {
    ...
    // When cubeSize is provided, size the wrapper to the grid; otherwise let it fill its container
    ...(cubeSize
      ? {
  ...
  return (
    <div className={`relative ${className ?? 'w-full h-full'}`} style={wrapperStyle}>
